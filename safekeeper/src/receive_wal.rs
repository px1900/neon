...
impl SafekeeperPostgresHandler {
    ...
    pub async fn handle_start_wal_push_guts<IO: AsyncRead + AsyncWrite + Unpin>(
        ...
        //! There are two step to persistent WAL:
        //! 1. Receive WAL from client via Network. (network_reader will do this)
        //! 2. Write WAL to disk. (WalAcceptor will do this)
        //! network_reader will send WAL to WalAcceptor via channel msg_tx/msg_rx.
        let (msg_tx, msg_rx) = channel(MSG_QUEUE_SIZE);
        //! The reply message will generated by WalAcceptor and send to network_writer via channel reply_tx/reply_rx.
        ...
        //! Split the socket to reader(network_reader::pgb_reader) and writer(pgb).
    ...
    async fn run(
        ...
        let tli = match next_msg {
            ProposerAcceptorMessage::Greeting(ref greeting) => {
                ...
                //! Create a new timeline with ttid, if exist, return the existing timeline.
...
impl WalAcceptor {
    ...
    async fn run(&mut self) -> anyhow::Result<()> {
        ...
        loop {
            ...
            //! If this message is a AppendRequest, then get into the loop
            //!     loop start:
            //!        change the AppendRequest to NoFlushAppendRequest. (don't need fsync)
            //!        send it to timeline
            //!        if next msg is AppendRequest, goto loop start
            //!        else goto loop end
            //!     loop end
            //! Send Flush message to timeline.
            //!
            //! In this way, we can write as many WAL as possible without fsyncing.
